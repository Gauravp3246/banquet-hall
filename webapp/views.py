...
# Showing Banquest list to Customer
def banquet(request):
...
def getAllCategories(request, banquet_id):
    # You might need to adjust this depending on your model relationships
    categories = Category.objects.all()
    context = {'categories': categories, 'banquet_id': banquet_id}
    return render(request, 'webapp/category.html', context)


#menu will come on click of category
def banquetMenu(request,banquet_id=None, category_id=None):

	menu = Menu.objects.filter(r_id=banquet_id,category_id=category_id)
	rest = Restaurant.objects.filter(id=banquet_id)
...
def checkout(request):
	if request.POST:
		...
		data = {
            "amount": totalprice *100,  # Razorpay expects the amount in the smallest currency unit, e.g., paise
            "currency": "INR",
            "payment_capture": '1'
        }
...
def menuManipulation(request):
	...
	if request.POST:
		...
		if type =="Modify":
			...
			caethoryid=int(request.POST['category'])
			category=Category.objects.filter(id=caethoryid)
			category=category[0]
...
#Data
 ...
 #Category 
	...
	categorys = []
	for z in categoryData:
		citem = []
		citem.append(z.id)
		citem.append(z.category_name)
		categorys.append(citem)
