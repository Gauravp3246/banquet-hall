...
class Restaurant(models.Model):
	...
	REST_AC_CHOICES =(
			(REST_AC,REST_AC),
			(REST_NO_AC,REST_NO_AC)
		)
	status 	= models.CharField(max_length=50,choices=REST_STATE_CHOICES,default=REST_STATE_OPEN,blank=False)
	ac 	= models.CharField(max_length=50,choices=REST_AC_CHOICES,default=REST_AC,blank=False)
...
class Menu(models.Model):
	...
	def __str__(self):
		return self.category_id.category_name + self.item_id.fname+' - '+str(self.price)
